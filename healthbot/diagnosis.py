...
# ────────────────────────────────────────────────────────────────
def generate_diagnosis(symptom: str):
    try:
        prompt = f"Patient presents with {symptom}. Possible diagnoses include"
        ...
        raw_items = [item.strip(" .") for item in result.split(",") if len(item.strip()) > 2]
        top_3 = raw_items[:3]

        explanations = []
        for item in top_3:
            clean_term = re.sub(r"\(.*?\)", "", item).strip()
            explanation = explanation_map.get(clean_term.lower(), "No explanation available.")
            explanations.append((item.title(), explanation))
...
# ────────────────────────────────────────────────────────────────
def generate_tips_from_biogpt(condition: str):
    try:
        prompt = f"What are the best health recommendations for managing {condition}?"
        inputs = tokenizer(prompt, return_tensors="pt").to(device)
        outputs = model.generate(**inputs, max_length=80, num_beams=5, early_stopping=True)
        response = tokenizer.decode(outputs[0], skip_special_tokens=True)

        # Clean and extract tips
        response = response.replace(prompt, "").strip().lower()
        tips = [tip.strip(" .") for tip in response.split(".") if len(tip.strip()) > 3]

        return tips[:5]  # Limit to 5 useful tips
